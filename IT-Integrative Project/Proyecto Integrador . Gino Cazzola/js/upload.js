...
// Descripcion corta *
    if (validateStrings(shortDescriptionInput.value)) {
        ...
        shortDescriptionInput.ariaInvalid = true
        shortDescriptionError.innerText = "Debe colocar los caracteres alfabéticos que corresponden."
        shortDescriptionError.style.display = "block"
        validForm = false
    }
 ...
 // Precio de juguetes*
    if (!validatePostiveNumber(priceInput.value)) {
        ...
        priceError.innerText = "Requiere ser un número positivo"
 ...
 // Stock de la tienda*
    if (!validatePostiveNumber(stockInput.value)) {
        ...
        stockError.innerText = "Requiere ser un número positivo"
        stockError.style.display = "block"
        validForm = false
    } else if (!validateInt(parseInt(stockInput.value))) {
        stockInput.ariaInvalid = true
        stockError.innerText = "Requiere ser un número entero"
        stockError.style.display = "block"
        validForm = false
    } else {
...
// Edad desde (mín) y hasta (máx)
    ...
    if (ageFromInput.value.length || ageToInput.value.length) { 
        if (!validatePostiveNumber(ageFromInput.value)) {
            ...
            ageFromError.innerText = "Debe ser un número positivo correspondiente"
            ageFromError.style.display = "block"
            validForm = false
        } else if (!validateInt(parseInt(ageFromInput.value))) {
            ageFromInput.ariaInvalid = true
            ageFromError.innerText = "Debe ser un número entero indicado"
            ageFromError.style.display = "block"
            validForm = false
        } else {
        ...
        if (!validatePostiveNumber(ageToInput.value)) {
            ...
            ageToError.innerText = "Debe ser un número positivo correspondiente"
            ageToError.style.display = "block"
            validForm = false
        } else if (!validateInt(parseInt(ageToInput.value))) {
            ageToInput.ariaInvalid = true
            ageToError.innerText = "Debe ser un número entero indicado"
            ageToError.style.display = "block"
            validForm = false
